...
def get_gemini_api_key():
    """Streamlit Secrets / 環境変数からGemini APIキーを読む"""
    try:
...
def build_gemini_prediction_prompt(date, place, race_no, race_name, surface, distance, use_day_bias=True):
...
with st.expander("Gemini APIで1レース自動予想", expanded=False):
    ...
    with c2:
        auto_race_name = st.text_input("レース名", value="", key="auto_race_name")
        auto_surface = st.selectbox("芝/ダ", ["芝", "ダ"], key="auto_surface")
        auto_distance = st.number_input("距離", min_value=1000, max_value=4000, value=1800, step=100, key="auto_distance")
    ...
    if st.button("全自動予想（馬場・TB検索込み）を実行", type="primary"):
        prompt = build_gemini_prediction_prompt(
            auto_date, auto_place, int(auto_race_no), auto_race_name,
            auto_surface, int(auto_distance), use_day_bias=use_day_bias
        )
        try:
            with st.spinner("Geminiで出走馬情報・馬場TBを検索してCSVを作成中です..."):
