...
def verified_axis_condition(row):
    """
    過去5年検証条件クリア判定。
    優先1: 直線替わり・前走内容判定_v2 がCSVにある場合は本命A/B/Cを使う。
    優先2: 21列CSVなどで直線替わり列が無い場合は、
            前走直線点・3角/4角位置・展開/直線評価で代替判定する。
    """
    change = norm_text(row.get("直線替わり", ""))
    content = norm_text(row.get("前走内容判定_v2", ""))

    p3 = normalize_existing_position_cat(
        row.get("prev3cCat", row.get("前3角位置カテゴリ", ""))
    )
    p4 = normalize_existing_position_cat(
        row.get("prev4cCat", row.get("前4角位置カテゴリ", ""))
    )

    straight_eval = norm_text(row.get("straightEval", row.get("直線相性評価", "")))
    pace_eval = norm_text(row.get("paceEval", row.get("展開予想評価", "")))

    prev_score = to_float(row.get("prevStraight", row.get("前走直線ロジック点", 0)), 0.0)
    total_score = to_float(row.get("総合点", 0), 0.0)

    good_mid_pos = {"4-6番手", "7-10番手"}
    good_front_mid_pos = {"2-3番手", "4-6番手", "7-10番手"}
    good_eval = {"向く", "かなり向く"}

    # =====================================================
    # 直線替わり・前走内容列がある場合：検証A/B/C条件を優先
    # =====================================================
    if change or content:
        # A: 長→短 × 先行粘り × 前4角2-3
        ...
        if change == "同タイプ" and content == "好位差し届かず":
            if p3 in good_mid_pos or p4 in good_mid_pos:
                return True, "本命C:同タイプ×好位差し届かず×中団"

    # =====================================================
    # 21列CSV用フォールバック：
    # 直線替わり列が無い場合でも、推奨馬がゼロにならないよう
    # 前走直線点・位置カテゴリ・評価語で判定する
    # =====================================================

    # D: 前走直線点が高く、中団〜好位で再現しやすく、直線/展開評価が向く
    if prev_score >= 60 and (p3 in good_mid_pos or p4 in good_mid_pos):
        if straight_eval in good_eval or pace_eval in good_eval:
            return True, "本命D:前走直線点60以上×中団好位×評価向く"

    # E: 前走直線点が高く、2-3番手〜7-10番手の実用位置にいる
    if prev_score >= 65 and (p3 in good_front_mid_pos or p4 in good_front_mid_pos):
        return True, "本命E:前走直線点65以上×実用位置"

    # F: 総合点が高く、評価語が揃っている
    if total_score >= 55 and straight_eval in good_eval and pace_eval in good_eval:
        return True, "本命F:総合点55以上×展開直線評価向く"
...
def render_rank_cards(g):
    ...
    for _, r in g.iterrows():
        try:
            horse_no = int(float(r["horseNo"])) if pd.notna(r["horseNo"]) else 0
        except Exception:
            horse_no = 0

        horse_name = norm_text(r.get("horseName", ""))
        rank = norm_text(r.get("トータルランク", ""))
        border = rank_border.get(rank, "#9aa9de")
        ...
        <div class="rank-row">
            ...
            <div class="rank-block">
                ...
                <div class="rank-badge" style="border-color:{border};">{rank}</div>
            </div>
        </div>
        """)

    date_text = norm_text(g.iloc[0].get("date", ""))
    race_text = norm_text(g.iloc[0].get("レース", ""))
    race_name = norm_text(g.iloc[0].get("raceName", ""))
    dist_text = norm_text(g.iloc[0].get("距離表示", ""))
    ...
    <body>
      <div class="wrap">
        <div class="title">{date_text} {race_text}</div>
        <div class="subtitle">{race_name} / {dist_text}</div>
        <div class="rows">
          {''.join(rows)}
        </div>
      </div>
...
st.caption("直線替わり列がある場合は検証A/B/C条件、無い場合は21列CSV用フォールバック条件で推奨馬を表示します。")
    ...
    with tab3:
        ...
        with col1:
            if st.button("3会場まとめSNS画像を作成"):
                ...
                if img is None:
                    st.warning("検証条件クリアの推奨馬はありません。CSVに直線替わり列が無い場合は、前走直線点・位置カテゴリ・評価語で判定します。")
                else:
