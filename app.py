...
st.set_page_config(page_title='競馬ランクアプリ v13.1 Buy Type Label Fix', layout='centered')
...
def make_sns_image(saved):
    ...
    for r in items:
        ...
        rr['買いタイプ表示'] = norm_text(rr.get('買いタイプ表示', '')) or bet_type_label_from_row(rr)
        cleaned.append(rr)
    ...
    buy_type_font = get_font(27, True)
    ...
    for i, r in enumerate(cleaned):
        ...
        buy_type_text = norm_text(r.get('買いタイプ表示', ''))
        ...
        draw_fit_text(draw, (340, y - 6), horse_name, horse_font, white, 440)
        if buy_type_text:
            draw_buy_type_pill(draw, 955, y + 4, buy_type_text, buy_type_font)
        if mate_text:
...
# UI
# =========================
st.title('競馬ランクアプリ v13.1 Buy Type Label Fix')
st.write('◎推奨馬ロジックは維持し、○▲は従来の上位評価寄り、△だけ「2着残り専用枠」に変更した完全版です。')
st.caption('v13.1: SNS画像右側に 連系 / 複系 / 激アツ を表示。古い保存データでも買い目数から自動推定します。')
    ...
    with tab2:
        for race_id in df['レース識別ID'].unique():
            ...
            current_recs.append({
                ...
                'おすすめ馬券': rec['bet_type'],
                '買いタイプ表示': bet_type_label(rec['bet_type']),
