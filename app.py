...
if st.session_state.generated_image_bytes is not None:
    ...
    st.markdown('<div class="preview-box"><b>過去レースCSVと予想CSVを読み込んでください</b></div>', unsafe_allow_html=True)
...
c1, c2 = st.columns(2)
with c1:
    st.markdown(f'<div class="metric-card"><div class="metric-label">過去データ件数</div><div class="metric-value">{history_count}</div></div>', unsafe_allow_html=True)
with c2:
