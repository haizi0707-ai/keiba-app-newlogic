...
st.set_page_config(page_title='競馬ランクアプリ v13.4 Recommendation CSV Export', layout='centered')
...
def recommend_for_race(g):
    ...
    # v12.9: 三連複フォーメーション3列目用の追加2頭
    extra_mates = _pick_extra_two(cand, used)

    mates = [m for m in [same, comp, ret] if m is not None]
    if len(mates) < 3:
        return {**base, 'status': '見送り', 'honmei_type': honmei_type, 'bet_strength': '見送り', 'bet_type': '見送り', 'bets': [], 'wide_bets': [], 'umaren_bets': [], 'trio_bets': [], 'mates': mates, 'extra_mates': [], 'reason': 'おすすめ馬券に必要な相手が揃わない'}

    same_reason = f'{_horse_label(same)}は本命と展開・位置取りが近い同展開相手。'
    comp_reason = f'{_horse_label(comp)}は本命と位置/展開にズレがあり、展開ズレを拾う補完相手。'
    ret_reason = f'{_horse_label(ret)}は低評価でも2着に残る形を拾う2着残り専用枠。'
    extra_reason = ''
    if extra_mates:
        extra_lines = []
        for role, m in extra_mates:
            extra_lines.append(f'{role}: {_horse_label(m)}')
        extra_reason = '\n追加2頭（三連複3列目用）：' + ' / '.join(extra_lines)

    d_count = sum(1 for m in mates if str(m.get('トータルランク', '')) == 'D')
    d_reason = ''
    if d_count >= 1:
        d_horses = ' / '.join([_horse_label(m) for m in mates if str(m.get('トータルランク', '')) == 'D'])
        d_reason = f'\nD評価採用理由：{d_horses}は2着残り専用枠限定で、位置取り/展開面のプラス材料を評価。'

    base_reason = f'本命は{type_reason}。\n' + same_reason + '\n' + comp_reason + '\n' + ret_reason + extra_reason + d_reason

    umaren_bets = [
        (_pair_text(honmei, same), '同展開相手'),
        (_pair_text(honmei, comp), '補完相手'),
        (_pair_text(honmei, ret), '2着残り専用枠'),
    ]
    trio_bets = _build_trio_bets(honmei, mates)
    strong_ok, strong_reason = _strong_buy_ok(honmei, conf, honmei_type, mates)

    if strong_ok:
        return {**base, 'status': '買い対象', 'honmei_type': honmei_type, 'bet_strength': '強気', 'bet_type': '馬連3点＋三連複3点', 'bets': umaren_bets + trio_bets, 'wide_bets': [], 'umaren_bets': umaren_bets, 'trio_bets': trio_bets, 'mates': mates, 'extra_mates': extra_mates, 'reason': base_reason + f'\n強気買い理由：{strong_reason}'}

    if honmei_type == '1・2着型':
        return {**base, 'status': '買い対象', 'honmei_type': honmei_type, 'bet_strength': '通常', 'bet_type': '馬連3点', 'bets': umaren_bets, 'wide_bets': [], 'umaren_bets': umaren_bets, 'trio_bets': [], 'mates': mates, 'extra_mates': extra_mates, 'reason': base_reason + '\n通常買い理由：本命が1・2着型のため馬連を優先。'}

    mate_ab_count = sum(1 for m in mates if str(m.get('トータルランク', '')) in ['S', 'A', 'B'])
    if mate_ab_count >= 2:
        return {**base, 'status': '買い対象', 'honmei_type': honmei_type, 'bet_strength': '通常', 'bet_type': '三連複3点', 'bets': trio_bets, 'wide_bets': [], 'umaren_bets': [], 'trio_bets': trio_bets, 'mates': mates, 'extra_mates': extra_mates, 'reason': base_reason + '\n通常買い理由：本命が3着型のため三連複で複勝力を活かす。'}

    wide_bets = [(_pair_text(honmei, same), '同展開相手'), (_pair_text(honmei, comp), '補完相手')]
    if ret is not None and str(ret.get('トータルランク', '')) != 'D':
        wide_bets.append((_pair_text(honmei, ret), '2着残り専用枠'))

    if len(wide_bets) < 2:
        return {**base, 'status': '見送り', 'honmei_type': honmei_type, 'bet_strength': '見送り', 'bet_type': '見送り', 'bets': [], 'wide_bets': [], 'umaren_bets': [], 'trio_bets': [], 'mates': mates, 'extra_mates': extra_mates, 'reason': '本命が3着型だが、ワイド/三連複の相手が不安定'}

    return {**base, 'status': '買い対象', 'honmei_type': honmei_type, 'bet_strength': '通常', 'bet_type': 'ワイド' + str(len(wide_bets)) + '点', 'bets': wide_bets, 'wide_bets': wide_bets, 'umaren_bets': [], 'trio_bets': [], 'mates': mates, 'extra_mates': extra_mates, 'reason': base_reason + '\n通常買い理由：本命が3着型かつ相手のまとまりが弱いためワイドを優先。'}
...
def _export_opponent_numbers(r, max_count=3):
    try:
        honmei_no = int(float(r.get('馬番', 0) or 0))
    except Exception:
        honmei_no = 0
...
def _format_marked_opponents(r):
    opponents = _export_opponent_numbers(r, max_count=3)
    marks = ['○', '▲', '△']
    parts = [f'{marks[i]}{opponents[i]}' for i in range(min(len(opponents), 3))]
...
def build_recommendation_export_df(records):
    """
    90%以上の推奨馬を、外部連携しやすい指定列CSVに整形する。
    出力列:
    日付,競馬場,R,レース名,馬番,馬名,信頼度,印,対抗,単穴,連下,他1,他2,相手表示
    """
    cols = ['日付', '競馬場', 'R', 'レース名', '馬番', '馬名', '信頼度', '印', '対抗', '単穴', '連下', '他1', '他2', '相手表示']
    if not records:
    ...
    for raw in records:
        r = dict(raw)
        try:
            conf = float(r.get('参考信頼度', r.get('信頼度', 0)) or 0)
        except Exception:
            conf = 0
        ...
        row = {
            '日付': r.get('日付', ''),
            '競馬場': norm_track(r.get('場所', r.get('競馬場', ''))),
            'R': r.get('R', ''),
            'レース名': r.get('レース名', r.get('raceName', '')),
            '馬番': horse_no,
            '馬名': r.get('馬名', ''),
            '信頼度': round(conf, 2),
            '印': '◎',
            '対抗': opponents[0] if len(opponents) > 0 else '',
            '単穴': opponents[1] if len(opponents) > 1 else '',
            '連下': opponents[2] if len(opponents) > 2 else '',
            '他1': r.get('他1', ''),
            '他2': r.get('他2', ''),
            '相手表示': _format_marked_opponents(r),
        }
        out_rows.append(row)
    ...
    # 並び順を安定化
    try:
        out['R_sort'] = pd.to_numeric(out['R'], errors='coerce')
        track_order = {'福島': 1, '東京': 2, '京都': 3, '阪神': 4, '中山': 5, '中京': 6, '新潟': 7, '小倉': 8, '札幌': 9, '函館': 10}
        out['track_sort'] = out['競馬場'].map(track_order).fillna(99)
        out = out.sort_values(['日付', 'track_sort', 'R_sort', '馬番']).drop(columns=['R_sort', 'track_sort'])
    except Exception:
        pass
...
# UI
# =========================
st.title('競馬ランクアプリ v13.4 Recommendation CSV Export')
st.write('◎推奨馬ロジックは維持し、○▲は従来の上位評価寄り、△だけ「2着残り専用枠」に変更した完全版です。')
st.caption('v13.4: ◎/○▲△/他1・他2の表示を維持。90%以上推奨馬を指定列CSVで出力できます。')
...
if uploaded is None:
    ...
    with tab3:
        ...
        if current_recs:
            ...
            current_export_df_90 = build_recommendation_export_df(current_recs)
            if not current_export_df_90.empty:
                current_export_csv_90 = current_export_df_90.to_csv(index=False, encoding='utf-8-sig')
                st.download_button(
                    'この会場の90%以上推奨馬CSVをダウンロード',
                    data=current_export_csv_90.encode('utf-8-sig'),
                    file_name='current_venue_recommendations_90plus_export.csv',
                    mime='text/csv'
                )
        ...
        if sdf.empty:
            ...
            export_df_90 = build_recommendation_export_df(st.session_state.saved_recs)
            if export_df_90.empty:
                st.info('90%以上の推奨馬CSV出力対象はまだありません。')
            else:
                st.dataframe(export_df_90, use_container_width=True, hide_index=True)
                export_csv_90 = export_df_90.to_csv(index=False, encoding='utf-8-sig')
                st.download_button(
                    '90%以上推奨馬CSVをダウンロード',
                    data=export_csv_90.encode('utf-8-sig'),
                    file_name='recommendations_90plus_export.csv',
                    mime='text/csv'
                )
