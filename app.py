...
def render_rank_cards(g):
    ...
    for _, r in g.iterrows():
        ...
        rows.append(f"""
        <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:20px;padding:12px 14px;margin:8px 0;">
          <div style="display:flex;align-items:center;gap:10px;min-width:0;flex:1;">
            <div style="font-size:15px;color:#aebee0;font-weight:700;min-width:20px;text-align:center;">{int(r["horseNo"])}</div>
            <div style="font-size:24px;color:white;font-weight:850;line-height:1.05;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{r["horseName"]}</div>
          </div>
          <div style="display:flex;flex-direction:column;align-items:center;gap:4px;flex-shrink:0;">
            <div style="font-size:12px;color:#b9c7e8;">ランク</div>
            <div style="width:50px;height:50px;border-radius:15px;border:3px solid {rank_color};display:flex;align-items:center;justify-content:center;color:white;font-weight:900;font-size:22px;">{r["トータルランク"]}</div>
          </div>
        </div>
        """)
    html = f"""
    <div style="background:#061734;border-radius:28px;padding:20px 18px 16px 18px;box-shadow:0 8px 24px rgba(0,0,0,0.18);">
