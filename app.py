...
def calc_relative_position(pos, field_size):
    if pd.isna(pos) or pd.isna(field_size):
        return np.nan
    try:
        pos = float(pos); field_size = float(field_size)
    except Exception:
        return np.nan
...
def map_rate_to_coef(rate, min_rate, max_rate):
    ...
    if q >= 0.80: return 1.25
    if q >= 0.60: return 1.10
    if q >= 0.40: return 1.00
    if q >= 0.20: return 0.85
    return 0.70
...
def assign_relative_ranks(df):
    out = df.copy(); out["相対評価"] = ""
    for race_id in out["レース識別ID"].unique():
        idx = out[out["レース識別ID"] == race_id].sort_values(["総合点","horseNo"], ascending=[False, True]).index.tolist()
        n = len(idx)
        if n == 0: continue
        s_n = max(1, round(n * 0.10)); a_n = max(1, round(n * 0.20)); b_n = max(1, round(n * 0.25)); tail_n = max(1, round(n * 0.15))
        for i, ix in enumerate(idx):
            if i < s_n: out.at[ix, "相対評価"] = "S"
            elif i < s_n + a_n: out.at[ix, "相対評価"] = "A"
            elif i < s_n + a_n + b_n: out.at[ix, "相対評価"] = "B"
            elif i < n - tail_n: out.at[ix, "相対評価"] = "C"
            else: out.at[ix, "相対評価"] = "D"
    return out

def _rank_score(v): return {"S": 5, "A": 4, "B": 3, "C": 2, "D": 1}.get(str(v), 0)
def _eval_score(v): return {"かなり向く": 5, "向く": 4, "普通": 3, "やや不向き": 2, "不向き": 1}.get(norm_text(v), 3)
def _position_score(v):
    s = normalize_existing_position_cat(v)
    return {"1番手":1,"2-3番手":2,"4-6番手":3,"7-10番手":4,"11番手以下":5}.get(s,3)
def _horse_label(row): return f'{int(row["horseNo"])} {row["horseName"]}'
def _pair_text(honmei, mate): return f'{int(honmei["horseNo"])} - {int(mate["horseNo"])}'
def _trio_text(honmei, a, b): return f'{int(honmei["horseNo"])} - {int(a["horseNo"])} - {int(b["horseNo"])}'
def _comment_text(row):
    vals = []
    for k in ["短評", "comment", "コメント", "評価コメント"]:
        if k in row.index: vals.append(norm_text(row.get(k, "")))
...
def judge_honmei_type(honmei, conf):
    rank = str(honmei.get("トータルランク", "")); score = float(honmei.get("総合点", 0) or 0); body = float(honmei.get("本体点", 0) or 0)
    pace = float(honmei.get("展開位置補正", 1) or 1); place = float(honmei.get("前走場所直線補正", 1) or 1)
    pace_eval = _eval_score(honmei.get("paceEval", "普通")); straight_eval = _eval_score(honmei.get("straightEval", "普通")); pos4 = _position_score(honmei.get("prev4cCat", "")); comment = _comment_text(honmei)
    win_words = ["勝ち切り", "押し切り", "主役", "軸上位", "能力上位", "前進", "頭まで", "連軸", "好位", "先行", "安定"]
    third_words = ["堅実", "相手向き", "複勝向き", "3着候補", "差し届けば", "展開待ち", "取りこぼし", "詰め甘い", "善戦型"]
    win_points = 0; third_points = 0
    if rank in ["S", "A"]: win_points += 2
    elif rank == "B": third_points += 1
    if conf >= 95: win_points += 2
    elif conf >= 90: third_points += 1
    if score >= 62: win_points += 2
    elif score < 52: third_points += 2
    if body >= 42: win_points += 1
    elif body < 36: third_points += 1
    if pace >= 1.08 and place >= 1.00: win_points += 1
    if pace < 1.00 or place < 1.00: third_points += 1
    if pace_eval >= 4 and straight_eval >= 3: win_points += 1
    if pace_eval <= 3 and straight_eval >= 3: third_points += 1
    if pos4 <= 3: win_points += 1
    elif pos4 >= 4: third_points += 1
...
def _prepare_candidates(g, honmei):
    ...
    h_pace = _eval_score(honmei.get("paceEval", "普通")); h_straight = _eval_score(honmei.get("straightEval", "普通")); h_3c = _position_score(honmei.get("prev3cCat", "")); h_4c = _position_score(honmei.get("prev4cCat", ""))
    cand["rank_score"] = cand["トータルランク"].apply(_rank_score); cand["relative_score"] = cand["相対評価"].apply(_rank_score); cand["pace_score"] = cand["paceEval"].apply(_eval_score); cand["straight_score"] = cand["straightEval"].apply(_eval_score); cand["pos3_score"] = cand["prev3cCat"].apply(_position_score); cand["pos4_score"] = cand["prev4cCat"].apply(_position_score)
    cand["same_gap"] = ((cand["pace_score"] - h_pace).abs() + (cand["straight_score"] - h_straight).abs() * 0.5 + (cand["pos3_score"] - h_3c).abs() * 0.7 + (cand["pos4_score"] - h_4c).abs() * 0.9)
    cand["same_score"] = cand["rank_score"] * 18 + cand["relative_score"] * 6 + cand["総合点"] * 0.70 - cand["same_gap"] * 10
    cand["diff_gap"] = ((cand["pace_score"] - h_pace).abs() + (cand["pos3_score"] - h_3c).abs() * 0.8 + (cand["pos4_score"] - h_4c).abs() * 1.0)
    ...
    cand["return_gap"] = ((cand["pace_score"] - h_pace).abs() + (cand["straight_score"] - h_straight).abs() + (cand["pos3_score"] - h_3c).abs() * 0.8 + (cand["pos4_score"] - h_4c).abs() * 0.8)
    cand["plus_comment"] = cand.apply(lambda r: 1 if any(k in _comment_text(r) for k in ["向く", "上積", "先行", "差し", "外", "内", "粘", "伸", "好位", "妙味", "穴"]) else 0, axis=1)
    cand["return_score"] = cand["return_gap"].clip(upper=5) * 5 + cand["rank_score"] * 9 + cand["relative_score"] * 4 + cand["総合点"] * 0.45 + cand["plus_comment"] * 8
    return cand

def _pick_same(cand, used):
    pool = cand[~cand["horseNo"].isin(used)].copy(); pool = pool[pool["トータルランク"].isin(["S", "A", "B"])].copy()
    if pool.empty: pool = cand[(~cand["horseNo"].isin(used)) & (cand["トータルランク"].astype(str) != "D")].copy()
    if pool.empty: return None
    return pool.sort_values(["same_score", "総合点", "horseNo"], ascending=[False, False, True]).iloc[0]

def _pick_comp(cand, used):
    pool = cand[~cand["horseNo"].isin(used)].copy(); pool = pool[pool["トータルランク"].isin(["S", "A", "B", "C"])].copy(); pool = pool[pool["総合点"] >= 35].copy()
    if pool.empty: pool = cand[(~cand["horseNo"].isin(used)) & (cand["トータルランク"].astype(str) != "D")].copy()
    if pool.empty: return None
...
def _strong_buy_ok(honmei, conf, honmei_type, mates):
    if conf < 95.0 or honmei_type != "1・2着型" or len(mates) < 3: return False, "強気条件未満"
    ab_count = sum(1 for m in mates if str(m.get("トータルランク", "")) in ["S", "A", "B"]); d_count = sum(1 for m in mates if str(m.get("トータルランク", "")) == "D"); avg_score = sum(float(m.get("総合点", 0) or 0) for m in mates)/len(mates)
    if ab_count >= 2 and d_count <= 1 and avg_score >= 42: return True, "本命信頼度95%以上かつ相手3頭の中にA/B評価が2頭以上"
...
def recommend_for_race(g):
    ...
    honmei = g.iloc[0]; conf = confidence_from_score(honmei["総合点"]); short_comment = f'本体{honmei["本体点"]:.1f}×展開{honmei["展開位置補正"]:.2f}×場所{honmei["前走場所直線補正"]:.2f}'
    base = {"honmei":honmei,"confidence":conf,"short_comment":short_comment}
    if conf < 90.0:
        return {**base,"status":"見送り","honmei_type":"対象外","bet_strength":"見送り","bet_type":"見送り","bets":[],"wide_bets":[],"umaren_bets":[],"trio_bets":[],"mates":[],"reason":"本命信頼度が90%未満"}
    honmei_type, type_reason = judge_honmei_type(honmei, conf); cand = _prepare_candidates(g, honmei)
    if cand.empty:
        return {**base,"status":"見送り","honmei_type":honmei_type,"bet_strength":"見送り","bet_type":"見送り","bets":[],"wide_bets":[],"umaren_bets":[],"trio_bets":[],"mates":[],"reason":"おすすめ馬券に必要な相手が揃わない"}
    used = {honmei["horseNo"]}; same = _pick_same(cand, used)
    if same is not None: used.add(same["horseNo"])
    comp = _pick_comp(cand, used)
    if comp is not None: used.add(comp["horseNo"])
    ret = _pick_return(cand, used, allow_d=True)
    if ret is not None: used.add(ret["horseNo"])
    mates = [m for m in [same, comp, ret] if m is not None]
    if len(mates) < 3:
        return {**base,"status":"見送り","honmei_type":honmei_type,"bet_strength":"見送り","bet_type":"見送り","bets":[],"wide_bets":[],"umaren_bets":[],"trio_bets":[],"mates":mates,"reason":"おすすめ馬券に必要な相手が揃わない"}
    same_reason = f'{_horse_label(same)}は本命と展開・位置取りが近い同展開相手。'; comp_reason = f'{_horse_label(comp)}は本命と位置/展開にズレがあり、展開ズレを拾う補完相手。'; ret_reason = f'{_horse_label(ret)}は違う勝ち筋で配当上振れを狙う回収相手。'
    d_count = sum(1 for m in mates if str(m.get("トータルランク", "")) == "D"); d_reason=""
    if d_count >= 1:
        d_horses = " / ".join([_horse_label(m) for m in mates if str(m.get("トータルランク", "")) == "D"]); d_reason = f'\nD評価採用理由：{d_horses}は回収相手枠限定で、短評/展開面のプラス材料を評価。'
    base_reason = f'本命は{type_reason}。\n' + same_reason + "\n" + comp_reason + "\n" + ret_reason + d_reason
    umaren_bets = [(_pair_text(honmei,same),"同展開相手"),(_pair_text(honmei,comp),"補完相手"),(_pair_text(honmei,ret),"回収相手")]
    trio_bets = _build_trio_bets(honmei,mates); strong_ok,strong_reason = _strong_buy_ok(honmei,conf,honmei_type,mates)
    if strong_ok:
        return {**base,"status":"買い対象","honmei_type":honmei_type,"bet_strength":"強気","bet_type":"馬連3点＋三連複3点","bets":umaren_bets+trio_bets,"wide_bets":[],"umaren_bets":umaren_bets,"trio_bets":trio_bets,"mates":mates,"reason":base_reason+f'\n強気買い理由：{strong_reason}'}
    if honmei_type == "1・2着型":
        return {**base,"status":"買い対象","honmei_type":honmei_type,"bet_strength":"通常","bet_type":"馬連3点","bets":umaren_bets,"wide_bets":[],"umaren_bets":umaren_bets,"trio_bets":[],"mates":mates,"reason":base_reason+"\n通常買い理由：本命が1・2着型のため馬連を優先。"}
    mate_ab_count = sum(1 for m in mates if str(m.get("トータルランク", "")) in ["S", "A", "B"])
    if mate_ab_count >= 2:
        return {**base,"status":"買い対象","honmei_type":honmei_type,"bet_strength":"通常","bet_type":"三連複3点","bets":trio_bets,"wide_bets":[],"umaren_bets":[],"trio_bets":trio_bets,"mates":mates,"reason":base_reason+"\n通常買い理由：本命が3着型のため三連複で複勝力を活かす。"}
    wide_bets = [(_pair_text(honmei,same),"同展開相手"),(_pair_text(honmei,comp),"補完相手")]
    if ret is not None and str(ret.get("トータルランク", "")) != "D": wide_bets.append((_pair_text(honmei,ret),"回収相手"))
    if len(wide_bets) < 2:
        return {**base,"status":"見送り","honmei_type":honmei_type,"bet_strength":"見送り","bet_type":"見送り","bets":[],"wide_bets":[],"umaren_bets":[],"trio_bets":[],"mates":mates,"reason":"本命が3着型だが、ワイド/三連複の相手が不安定"}
    return {**base,"status":"買い対象","honmei_type":honmei_type,"bet_strength":"通常","bet_type":"ワイド"+str(len(wide_bets))+"点","bets":wide_bets,"wide_bets":wide_bets,"umaren_bets":[],"trio_bets":[],"mates":mates,"reason":base_reason+"\n通常買い理由：本命が3着型かつ相手のまとまりが弱いためワイドを優先。"}
...
def render_rank_cards(g):
    ...
    for _, r in g.iterrows():
        ...
        rows.append(f'''
        <div class="horse-row"><div class="horse-left"><div class="horse-no">{int(r["horseNo"])}</div><div class="horse-name">{r["horseName"]}</div></div><div class="rank-wrap"><div class="rank-label">ランク</div><div class="rank-box" style="border-color:{rank_color};">{r["トータルランク"]}</div></div></div>
        ''')
    html=f'''
    ...
    .card{{background:#061734;border-radius:28px;padding:20px 18px 16px 18px;box-shadow:0 8px 24px rgba(0,0,0,0.18);color:white;box-sizing:border-box;width:100%;}}
    .title{{font-size:23px;font-weight:900;color:white;line-height:1.2;}} .subtitle{{font-size:15px;color:#c3d0e8;margin-top:6px;}} .rows{{margin-top:14px;}}
    .horse-row{{display:flex;align-items:center;justify-content:space-between;gap:12px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:20px;padding:12px 14px;margin:8px 0;box-sizing:border-box;}}
    .horse-left{{display:flex;align-items:center;gap:10px;min-width:0;flex:1;}} .horse-no{{font-size:15px;color:#aebee0;font-weight:700;min-width:20px;text-align:center;}} .horse-name{{font-size:24px;color:white;font-weight:850;line-height:1.05;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}}
    .rank-wrap{{display:flex;flex-direction:column;align-items:center;gap:4px;flex-shrink:0;}} .rank-label{{font-size:12px;color:#b9c7e8;}} .rank-box{{width:50px;height:50px;border-radius:15px;border:3px solid #7b8db7;display:flex;align-items:center;justify-content:center;color:white;font-weight:900;font-size:22px;box-sizing:border-box;}}
    </style></head><body><div class="card"><div class="title">{g.iloc[0]["date"]} {g.iloc[0]["レース"]}</div><div class="subtitle">{g.iloc[0]["raceName"]} / {g.iloc[0]["距離表示"]}</div><div class="rows">{''.join(rows)}</div></div></body></html>
    '''
    height=190+len(g)*112
    components.html(html,height=height,scrolling=False)

def get_font(size,bold=False):
    candidates=["/usr/share/fonts/opentype/noto/NotoSansCJK-Bold.ttc" if bold else "/usr/share/fonts/opentype/noto/NotoSansCJK-Regular.ttc","/usr/share/fonts/opentype/noto/NotoSansJP-Bold.otf" if bold else "/usr/share/fonts/opentype/noto/NotoSansJP-Regular.otf","/usr/share/fonts/truetype/noto/NotoSansCJK-Bold.ttc" if bold else "/usr/share/fonts/truetype/noto/NotoSansCJK-Regular.ttc","/usr/share/fonts/truetype/noto/NotoSansJP-Bold.ttf" if bold else "/usr/share/fonts/truetype/noto/NotoSansJP-Regular.ttf","/usr/share/fonts/truetype/fonts-japanese-gothic.ttf","/usr/share/fonts/truetype/arphic-bkai00mp/bkai00mp.ttf","/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf" if bold else "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf"]
    for p in candidates:
...
def fit_text_to_width(draw,text,font,max_width):
    ...
    while len(t)>0:
        ...
        if bbox[2]-bbox[0]<=max_width: return t
        t=t[:-1]
    return ""

def draw_centered_text(draw,box,text,font,fill):
    x1,y1,x2,y2=box; t=str(text); bbox=draw.textbbox((0,0),t,font=font); tw=bbox[2]-bbox[0]; th=bbox[3]-bbox[1]
    x=x1+(x2-x1-tw)/2-bbox[0]; y=y1+(y2-y1-th)/2-bbox[1]
    draw.text((x,y),t,font=font,fill=fill)

def extract_opponent_marks(r):
    try: honmei_no=int(float(r.get("馬番",0) or 0))
    except Exception: honmei_no=0
    opponents=[]
    for key in ["買い目1","買い目2","買い目3","買い目4","買い目5","買い目6"]:
        ...
        for n in nums:
            ...
            if num<=0: continue
            if honmei_no and num==honmei_no: continue
            if num not in opponents: opponents.append(num)
...
def make_sns_image(saved):
    ...
    def clean_item(r):
        rr=dict(r); place_raw=norm_text(rr.get("場所","")); race_raw=norm_text(rr.get("レース","")); single_raw=norm_text(rr.get("単複おすすめ1",""))
        ...
        if m: rr["場所"]=m.group(1); rr["R"]=int(m.group(2))
        else:
            rr["場所"]=norm_track(place_raw)
            try: rr["R"]=int(float(rr.get("R",0) or 0))
            except Exception: rr["R"]=0
        try: rr["馬番"]=int(float(rr.get("馬番",0) or 0))
        except Exception:
            m2=re.search(r"(\d+)",single_raw); rr["馬番"]=int(m2.group(1)) if m2 else 0
        rr["馬名"]=norm_text(rr.get("馬名","")) or re.sub(r"^\d+\s*","",single_raw)
        rr["参考信頼度"]=float(rr.get("参考信頼度",0) or 0); rr["相手表示"]=extract_opponent_marks(rr)
        return rr
    items=[clean_item(r) for r in items]
    track_order={"福島":1,"東京":2,"京都":3,"阪神":4,"中山":5,"中京":6,"新潟":7,"小倉":8,"札幌":9,"函館":10}
    ...
    raw_date=str(items[0].get("日付","")); dt=pd.to_datetime(raw_date,errors="coerce")
    date_main=raw_date.replace("/",".").replace("-",".") if pd.isna(dt) else dt.strftime("%Y.%m.%d"); weekday="" if pd.isna(dt) else dt.strftime("%A").upper()
    W=1080; row_y=360; row_h=155; bottom_margin=125; H=max(1920,row_y+len(items)*row_h+bottom_margin)
    img=Image.new("RGB",(W,H),(10,14,22)); draw=ImageDraw.Draw(img); white=(248,248,244); gold=(229,191,72); muted=(145,149,160); line=(42,47,57); circle_line=(48,40,28); bg=(10,14,22)
    for offset,width in [(0,2),(55,2)]: draw.ellipse((680+offset,-120+offset,1260-offset,460-offset),outline=circle_line,width=width)
    small_eng_font=get_font(25,True); title_font=get_font(70,True); date_font=get_font(40,False); weekday_font=get_font(22,True); place_font=get_font(31,True); race_font=get_font(36,True); horse_no_font=get_font(42,True); horse_font=get_font(45,True); mate_font=get_font(25,False)
    draw.text((80,92),"T O D A Y ' S   P I C K S",font=small_eng_font,fill=gold); draw.text((80,150),"本日の推奨馬",font=title_font,fill=white); draw.text((670,135),date_main,font=date_font,fill=gold)
    if weekday: draw.text((780,190),weekday,font=weekday_font,fill=muted)
    draw.line((80,285,1000,285),fill=line,width=2)
    for i,r in enumerate(items):
        y=row_y+i*row_h
        if i>0: draw.line((80,y-42,1000,y-42),fill=(31,36,45),width=2)
        place=norm_track(r.get("場所","")); race_no=int(float(r.get("R",0) or 0)); horse_no=int(float(r.get("馬番",0) or 0)); horse_name=norm_text(r.get("馬名","")); mate_text=norm_text(r.get("相手表示",""))
        draw.text((80,y-6),place,font=place_font,fill=gold); draw.text((80,y+34),f"{race_no}R",font=race_font,fill=gold)
        cx,cy,rad=250,y+43,42; draw.ellipse((cx-rad,cy-rad,cx+rad,cy+rad),fill=gold); draw_centered_text(draw,(cx-rad,cy-rad,cx+rad,cy+rad),str(horse_no),horse_no_font,bg)
        name_x=340; draw_fit_text(draw,(name_x,y-6),horse_name,horse_font,white,620)
        if mate_text:
            mate_text_spaced=mate_text.replace("◯","○ ").replace("▲","▲ ").replace("△","△ "); draw.text((name_x,y+58),mate_text_spaced,font=mate_font,fill=muted)
...
def add_saved_recs(new_recs):
    if "saved_recs" not in st.session_state: st.session_state.saved_recs=[]
    cleaned=[]
    for r in new_recs:
        rr=dict(r); place_raw=norm_text(rr.get("場所","")); m=re.search(r"(福島|新潟|東京|中山|中京|京都|阪神|小倉|札幌|函館)\s*(\d+)\s*R",place_raw)
        if m: rr["場所"]=m.group(1); rr["R"]=int(m.group(2))
        else:
            rr["場所"]=norm_track(place_raw)
            try: rr["R"]=int(float(rr.get("R",0) or 0))
    ...
    for r in st.session_state.saved_recs:
        rr=dict(r); place=norm_track(norm_text(rr.get("場所","")))
        try: race_no=int(float(rr.get("R",0) or 0))
        except Exception: race_no=0
        rr["場所"]=place; rr["R"]=race_no; store[f'{rr.get("日付","")}_{place}_{race_no}R']=rr
    for r in cleaned: store[f'{r["日付"]}_{r["場所"]}_{r["R"]}R']=r
...
# 第3アプリ用：直線ロジックCSV出力
# =========================
GRADE_MARKERS=["G1","G2","G3","Ｇ１","Ｇ２","Ｇ３","GI","GII","GIII","ＧⅠ","ＧⅡ","ＧⅢ","Jpn1","Jpn2","Jpn3","Ｊｐｎ１","Ｊｐｎ２","Ｊｐｎ３"]
NON_GRADE_WORDS=["未勝利","新馬","1勝","２勝","2勝","３勝","3勝","500万","1000万","1600万","オープン","OP","ＯＰ","Ｌ","L","リステッド"]
GRADE_RACE_HINTS=["記念","杯","賞","ステークス","Ｓ","S","カップ","Ｃ","C","ダービー","オークス","皐月賞","菊花賞","桜花賞","天皇賞","有馬記念","宝塚記念","安田記念","マイルCS","マイルＣＳ","ヴィクトリアマイル","フェブラリーS","フェブラリーＳ","チャンピオンズC","チャンピオンズＣ","高松宮記念","スプリンターズS","スプリンターズＳ","ホープフルS","ホープフルＳ"]

def auto_grade_race_flag(row):
    values=[]
    for key in ["grade","グレード","重賞","クラス","raceName","レース名"]:
        if key in row.index: values.append(norm_text(row.get(key,"")))
    s=" ".join(values)
    if any(m in s for m in GRADE_MARKERS): return True
    if "重賞" in s: return True
    race_name=norm_text(row.get("raceName","")) or norm_text(row.get("レース名",""))
    if any(w in race_name for w in NON_GRADE_WORDS): return False
    try: rno=int(float(row.get("raceNo",0) or 0))
    except Exception: rno=0
    if rno in [10,11,12] and any(w in race_name for w in GRADE_RACE_HINTS): return True
    return False

def straight_rank_to_uratori(rank):
    rank=str(rank)
    if rank in ["S","A"]: return "強い"
    if rank=="B": return "普通"
    if rank=="C": return "弱い"
    if rank=="D": return "逆風"
    return "普通"
...
def build_third_app_straight_export(df, selected_race_ids):
    ...
    export_df=export_df.sort_values(["日付","場所","R","馬番"]).reset_index(drop=True)
    return export_df
...
if uploaded is None:
    ...
    with tab1:
        for race_id in df["レース識別ID"].unique():
            ...
            render_rank_cards(g); st.divider()
    with tab2:
        for race_id in df["レース識別ID"].unique():
            g=df[df["レース識別ID"]==race_id].sort_values(["総合点","horseNo"],ascending=[False,True]).reset_index(drop=True)
            st.subheader(f'{g.iloc[0]["date"]} {g.iloc[0]["レース"]} {g.iloc[0]["raceName"]}')
            rec=recommend_for_race(g); honmei=rec["honmei"]; conf=rec["confidence"]
            st.markdown("### 単複おすすめ1"); st.write(f'候補: {int(honmei["horseNo"])} {honmei["horseName"]}'); st.caption(f'トータル{honmei["トータルランク"]} / 総合点 {honmei["総合点"]:.2f} / 参考信頼度 {conf:.2f}% / {rec["short_comment"]}')
            st.markdown("### おすすめ馬券")
            if rec["status"]=="見送り":
                st.write("見送り"); st.caption(f'理由：{rec["reason"]}')
            else:
                st.write(f'本命タイプ：{rec["honmei_type"]}'); st.write(f'勝負度：{rec["bet_strength"]}'); st.write(f'おすすめ馬券：{rec["bet_type"]}')
                if rec.get("umaren_bets"):
                    st.write("馬連：")
                    for bet,role in rec["umaren_bets"]: st.write(f'{bet}　{role}')
                if rec.get("trio_bets"):
                    st.write("三連複：")
                    for bet,role in rec["trio_bets"]: st.write(bet)
                if rec.get("wide_bets"):
                    st.write("ワイド：")
                    for bet,role in rec["wide_bets"]: st.write(f'{bet}　{role}')
                st.caption("理由：\n"+rec["reason"])
            st.divider()
            bet_values=[b[0] for b in rec.get("bets",[])]
            current_recs.append({"日付":honmei["date"],"場所":honmei["場所"],"R":safe_race_no(honmei),"馬番":int(honmei["horseNo"]),"馬名":honmei["horseName"],"単複おすすめ1":f'{int(honmei["horseNo"])} {honmei["horseName"]}',"参考信頼度":round(float(conf),2),"短評":rec["short_comment"],"買い対象":1 if rec["status"]=="買い対象" else 0,"本命タイプ":rec["honmei_type"],"勝負度":rec["bet_strength"],"おすすめ馬券":rec["bet_type"],"買い目1":bet_values[0] if len(bet_values)>0 else "","買い目2":bet_values[1] if len(bet_values)>1 else "","買い目3":bet_values[2] if len(bet_values)>2 else "","買い目4":bet_values[3] if len(bet_values)>3 else "","買い目5":bet_values[4] if len(bet_values)>4 else "","買い目6":bet_values[5] if len(bet_values)>5 else "","相手選定理由":rec["reason"]})
    with tab3:
        st.subheader("この会場の推奨馬")
        if current_recs: st.dataframe(pd.DataFrame(current_recs),use_container_width=True,hide_index=True)
        if st.button("この会場の推奨馬を保存",type="primary"):
            add_saved_recs(current_recs); st.success("この会場の単複おすすめ1を保存しました。")
        st.subheader("保存済み推奨馬")
        sdf=saved_df()
        if sdf.empty: st.info("まだ保存済み推奨馬はありません。")
        else:
        ...
        st.caption("複数レースを読み込んでいる場合でも、ここで選択した重賞候補レースだけを第3アプリ用CSVとして出力します。")
        ...
        selected_grade_races=st.multiselect("第3アプリへ出力する重賞レースを選択",options=race_options_df["レース識別ID"].tolist(),default=default_ids,format_func=lambda x: race_options_df.loc[race_options_df["レース識別ID"]==x,"表示名"].iloc[0])
        ...
        with col1:
            if st.button("3会場まとめSNS画像を作成"):
                ...
                if img is None: st.warning("信頼度90%以上の推奨馬はありません")
                else:
        ...
        with col2:
            if st.button("保存済み推奨馬をクリア"):
                st.session_state.saved_recs=[]; st.success("保存済み推奨馬をクリアしました。")
    with tab4:
        ...
        show_df=df[check_cols].rename(columns={"date":"日付","horseNo":"馬番","horseName":"馬名","prevFieldSize":"前走頭数","prev3cPos":"前3角通過順","prev4cPos":"前4角通過順","prev3cRel_final":"前3角相対位置","prev4cRel_final":"前4角相対位置","prev3cCat_original":"前3角元カテゴリ","prev4cCat_original":"前4角元カテゴリ","prev3cCat":"前3角補正後カテゴリ","prev4cCat":"前4角補正後カテゴリ"})
        ...
        export_df=df[export_cols].rename(columns={"date":"日付","raceName":"レース名","horseNo":"馬番","horseName":"馬名","prevFieldSize":"前走頭数","prev3cPos":"前3角通過順","prev4cPos":"前4角通過順","prev3cRel_final":"前3角相対位置","prev4cRel_final":"前4角相対位置","prev3cCat":"前3角位置カテゴリ","prev4cCat":"前4角位置カテゴリ"})
        result_csv=export_df.to_csv(index=False,encoding="utf-8-sig")
        st.download_button("補正確認付き予想結果CSVをダウンロード",data=result_csv.encode("utf-8-sig"),file_name="keiba_rank_v122_relative_results.csv",mime="text/csv")
