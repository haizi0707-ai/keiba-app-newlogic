...
function buildHistorySummary(rows){
  const maps = {
    sire_track:new Map(), sire_dist:new Map(), sire_going:new Map(),
    trainer_track:new Map(), trainer_dist:new Map(), trainer_interval:new Map(),
    trainer_distchg:new Map(), trainer_trackchg:new Map()
  };
  const place = (r) => {
    const x = Number(r["着順"]);
    return x >= 1 && x <= 3 ? 1 : 0;
  ...
  const add = (map, key, p) => {
    if(!map.has(key)) map.set(key,{count:0, place:0});
    const obj = map.get(key); obj.count += 1; obj.place += p;
  };
  ...
  rows.forEach(r => {
    ...
    add(maps.sire_track, `${sire}|||${track}`, p);
    add(maps.sire_dist, `${sire}|||${distBand}`, p);
    add(maps.sire_going, `${sire}|||${going}`, p);

    add(maps.trainer_track, `${trainer}|||${track}`, p);
    add(maps.trainer_dist, `${trainer}|||${distBand}`, p);
    add(maps.trainer_interval, `${trainer}|||${interval}`, p);
    add(maps.trainer_distchg, `${trainer}|||${distchg}`, p);
    add(maps.trainer_trackchg, `${trainer}|||${trchg}`, p);
  });

  return maps;
}

function lookupScore(map, key, minCount=10){
  const x = map.get(key);
  if(!x || x.count < minCount) return {score:null, count:null};
  return {score:(x.place / x.count) * 100, count:x.count};
}
...
function updateMetrics(){
  document.getElementById('mHistory').textContent = historyRows.length;
  const condCount = historyRows.length ? Object.keys(buildHistorySummary(historyRows)).reduce((a,k)=>a+buildHistorySummary(historyRows)[k].size,0) : 0;
...
document.getElementById('loadHistoryBtn').addEventListener('click', async () => {
  ...
  if(!file){
    showStatus('historyStatus','過去レースCSVを選択してください。','error');
    return;
  }
  const text = await readFileAsText(file);
  historyRows = parseCSV(text);
  localStorage.setItem('keiba_history_rows', JSON.stringify(historyRows));
  showStatus('historyStatus', `過去データを取り込みました。件数: ${historyRows.length}`, 'success');
  updateMetrics();
});

document.getElementById('backupBtn').addEventListener('click', () => {
  const blob = new Blob([JSON.stringify({records:historyRows}, null, 2)], {type:'application/json'});
  const a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
  a.download = 'keiba_history_backup.json';
  a.click();
});

document.getElementById('clearHistoryBtn').addEventListener('click', () => {
  historyRows = [];
  localStorage.removeItem('keiba_history_rows');
  showStatus('historyStatus','過去データを削除しました。','success');
  updateMetrics();
});
...
document.getElementById('loadPredictionBtn').addEventListener('click', async () => {
  ...
  if(!historyRows.length){
    showStatus('predictionStatus','先に過去レースCSVを取り込んでください。','error');
    return;
  }

  const text = await readFileAsText(file);
  predictionRows = parseCSV(text);
  ...
  rankedRows = predictionRows.map(r => {
    ...
    const s1 = lookupScore(summary.sire_track, `${sire}|||${track}`);
    const s2 = lookupScore(summary.sire_dist, `${sire}|||${distBand}`);
    const s3 = lookupScore(summary.sire_going, `${sire}|||${going}`);

    const t1 = lookupScore(summary.trainer_track, `${trainer}|||${track}`);
    const t2 = lookupScore(summary.trainer_dist, `${trainer}|||${distBand}`);
    const t3 = lookupScore(summary.trainer_interval, `${trainer}|||${interval}`);
    const t4 = lookupScore(summary.trainer_distchg, `${trainer}|||${distchg}`);
    const t5 = lookupScore(summary.trainer_trackchg, `${trainer}|||${trchg}`);
...
(function init(){
  const saved = localStorage.getItem('keiba_history_rows');
  if(saved){
    try {
      historyRows = JSON.parse(saved);
      if(Array.isArray(historyRows) && historyRows.length){
        showStatus('historyStatus', `保存済みの過去データを読み込みました。件数: ${historyRows.length}`, 'success');
      }
    } catch(e){}
  }
  updateMetrics();
