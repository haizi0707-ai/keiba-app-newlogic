...
st.set_page_config(page_title='競馬ランクアプリ v13.2 Standard Bet Display', layout='centered')
...
def make_sns_image(saved):
    ...
    bet_display_font = get_font(23, True)
    ...
    for i, r in enumerate(cleaned):
        ...
        draw_fit_text(draw, (340, y - 6), horse_name, horse_font, white, 415)
        if bet_display_text:
            draw_bet_display_box(draw, 995, y - 2, bet_display_text, bet_display_font)
        if mate_text:
...
# UI
# =========================
st.title('競馬ランクアプリ v13.2 Standard Bet Display')
st.write('◎推奨馬ロジックは維持し、○▲は従来の上位評価寄り、△だけ「2着残り専用枠」に変更した完全版です。')
st.caption('v13.2: SNS画像右側に標準型の買い目を表示。馬連は◎-○▲△、ワイドは◎-○△を表示します。')
    ...
    with tab2:
        for race_id in df['レース識別ID'].unique():
            ...
            current_recs.append({
                ...
                'おすすめ馬券': rec['bet_type'],
                '買い目表示': build_standard_bet_display({
                    '馬番': int(honmei['horseNo']),
